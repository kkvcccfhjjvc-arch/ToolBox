...
async def lab_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lab_id == "developer":
        from bot.labs.developer.handler import developer_lab_menu
        await developer_lab_menu(update, context)
        return

    if lab_id == "utility":
        from bot.labs.utility.handler import utility_lab_menu
        await utility_lab_menu(update, context)
        return
